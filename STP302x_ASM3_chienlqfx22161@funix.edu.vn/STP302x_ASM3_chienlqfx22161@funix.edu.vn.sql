...
select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Bắt Đầu', PB.TenPB as 'Phòng Ban' from nhanvien NV
join phongban PB on NV.MaPB = PB.MaPB
where date (NgayVao) >= 20210308 ;
...
select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Bắt Đầu', PB.TenPB as 'Phòng Ban', HD.LoaiHD as 'Loại Hợp Đồng', HD.TuNgay as 'Ngày Ký' from nhanvien NV
join phongban PB on NV.MaPB=PB.MaPB
join hopdonglaodong HD on nv.MaNV=HD.MaNV
where date (HD.TuNgay) > 20180301
and HD.LoaiHD = 'Chính thức';

select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Bắt Đầu', BL.LuongCoBan as 'Lương Cơ Bản', BL.PhuCap as 'Phụ Cấp' from nhanvien NV
join phongban PB on NV.MaPB = PB.MaPB
join bangluong BL on NV.MaBangLuong = BL.MaBangLuong
where PB.TenPB = 'Phòng Hành Chính'
and NV.ChucVu = 'Trường Phòng';

Select PB.TenPB as 'Tên Phòng Ban', count(NV.HoTen) as 'Số Lượng Nhân Viên' from nhanvien NV
join phongban PB on NV.MaPB=PB.MaPB
where PB.TenPB = 'Phòng Kế Hoạch';

select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Vào Công Ty', PB.TenPB as 'Phòng Ban', round(datediff(curdate(),NV.NgayVao)/365) as 'Số Năm Làm Việc' from nhanvien NV
join phongban PB on NV.MaPB=PB.MaPB
where round(datediff(curdate(),NV.NgayVao)/365)>=4;

update nhanvien
set NgaySinh = '1988-03-15'
where MaNv = 31019;
select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Vào Công Ty', PB.TenPB as 'Phòng Ban' from nhanvien NV
join phongban PB on NV.MaPB=PB.MaPB
where MaNv = 31019;

select NV.MaNv AS 'Mã Nhân Viên', NV.HoTen AS 'Họ Tên', NV.NgaySinh AS 'Ngày Sinh', NV.QueQuan AS 'Quê Quán', NV.SoDienThoai AS 'Số Điện Thoại', NV.ChucVu AS 'Chức Vụ', NV.NgayVao AS 'Ngày Vào Công Ty', PB.TenPB as 'Phòng Ban' from nhanvien NV
join phongban PB on NV.MaPB=PB.MaPB
where right(NV.HoTen,3) = 'Nam';
...
select PB.TenPB as 'Tên Bộ Phận', NV.QueQuan as 'Tỉnh Thành', count(NV.QueQuan) as 'Số Nhân Viên' from nhanvien NV
join phongban PB on NV.MaPB = PB.MaPB
group by PB.TenPB, NV.QueQuan;

select PB.TenPB as 'Tên Bộ Phận', round(avg(BL.LuongCoBan+BL.PhuCap),2) as 'Lương Trung Bình'
from bangluong BL
join nhanvien NV on BL.MaBangLuong = NV.MaBangLuong
join phongban PB on NV.MaPB = PB.MaPB
group by PB.MaPB;
